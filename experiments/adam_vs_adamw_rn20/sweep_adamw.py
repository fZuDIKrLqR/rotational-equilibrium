...
--model cifar_resnet --model-kwargs 'name=cifar_rn20' 'pre_activation=False' \
...
--log-wandb --wandb-kwargs project=melo_iclr_adam_vs_adamw_rn20 name="\"rn20_adamw_iter${iter}_lr${lr}_wd${wd}_seed${seed}\"" --seed $seed
"""

# Configure runs
def main():
    # Sweep is centered on RN20 hyperparameters from Table 3 (first version)
    # Base lr adjusted by 2x to account for batch size
    base_dir = os.getcwd()
    lrs = [0.1 * 2**i for i in range(-5, 6)]
    wds = [0] + [1e-2 * 2**i for i in range(-5, 6)]
